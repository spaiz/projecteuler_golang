...
https://www.xarg.org/puzzle/project-euler/problem-11/
https://euler.beerbaronbill.com/en/latest/solutions/11.html
*/
func main() {
 ...
 */
func GreatestProduct1(m [][]int, length int) *Result {
	maxResult := &Result{}

	for i := 0; i < len(m) ; i++ {
		for j := 0; j < len(m); j++ {
			res := FindLocalMaximumProduct(m, i, j, length)
			if res.product > maxResult.product {
				maxResult = res
			}
		}
	}
